...
$(function(){
   //웹소켓 시작
   const message_socket = new WebSocket("ws://localhost:8080/COZYTRIP/webSocket");
         ...
         success: function(param) {
            if(param.result == 'logout') {
               ...
               $(param.list).each(function(index, item) {
            	   let output = '<div class="room-item" data-chatroom-num="' + item.chatroom_num + '" data-accom-num="' + item.accom_num + '">';
               ...
               $('.rooms-list').off('click', '.room-item').on('click', '.room-item', function() {
            	   const chatroomNum = $(this).data('chatroom-num');
                   location.href = '${pageContext.request.contextPath}/host/chattingDetail.do?chatroom_num=' + chatroomNum;
                });
   ...
   $('#room-search').on('input', function() {
      ...
      $('.room-item').each(function() {
         const titleText = $(this).find('.room-title').text().toLowerCase();
         const accomText = $(this).find('.room-accom').text().toLowerCase();
         
         if(titleText.includes(searchText) || accomText.includes(searchText)) {
            $(this).show();
         } else {
            $(this).hide();
         }
   ...
   function selectData(){
      //서버와 통신
      $.ajax({
         url:'${pageContext.request.contextPath}/accom/chatMessageList.do',
         type:'post',
         ...
         success:function(param){
            ...
            $(param.list).each(function(index,item){
               ...
               if(item.user_num == ${user_num}){//일치
                  output += '<div class="from-position">'
               }else{//불일치
                  output += '<div class="to-position">' + item.user_id;
               }
   ...
   $('#chatting_form').submit(function(event){
      ...
      //서버와 통신
      $.ajax({
         url:'${pageContext.request.contextPath}/accom/chatWrite.do',
         type:'post',
         data:form_data,
         dataType:'json',
         success:function(param){
...
<div class="page-main">
    ...
    <div class="content-main chat-wrapper">
        <h2>
            ${AccomChatRoom.user_id}</small>님과 채팅
        </h2>
